...
def run(connection_manager=None, state=None):
    ...
    ╚══════════════════════════════════════════════════════════╝
    """)
    if state and state.is_completed("user_locations"):
        logger.info("User locations migration already completed (resume).")
        return True
    ...
    try:
        ...
        if success:
            if state:
                state.clear_step("user_locations")
            print("\n" + "="*60)
        ...
        if ConnectionLostError:
            raise ConnectionLostError("user_locations", {}) from e
        raise
    except Exception as e:
